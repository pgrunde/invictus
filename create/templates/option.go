...
func CreateOption(projectName, fullpath string) {
	file, err := os.Create(fmt.Sprintf("%s/%s/server/api/option.go", fullpath, projectName))
	if err != nil {
		log.Fatalf("cannot create a server/api/option.go file: %s", err)
	}
	defer file.Close()
	optionTemplate.Execute(file, nil)
}
